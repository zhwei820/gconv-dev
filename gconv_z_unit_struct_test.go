...
func Test_Struct_Attr_CustomType2(t *testing.T) {
	...
	gtest.C(t, func(t *gtest.T) {
		user := new(User)
		err := gconv.Struct(map[string]interface{}{"id": []interface{}{1, 2}, "name": "john"}, user)
		t.Assert(err, nil)
		t.Assert(user.Id, []interface{}{1, 2})
		t.Assert(user.Name, "john")
	})
}
...
// https://github.com/zhwei820/gconv/issues/775
...
func Test_Struct_Complex(t *testing.T) {
    ...
    "errorMsg": null
		...
		err := gconv.UnmarshalUseNumber([]byte(data), &m)
		t.Assert(err, nil)
...
func Test_Struct_Slice(t *testing.T) {
	gtest.C(t, func(t *gtest.T) {
		...
		err := gconv.Struct(map[string]interface{}{"scores": array}, user)
		t.Assert(err, nil)
